...
/*void TranceFile(struct Package package)
{
	int ifrecv;
...
//		strcpy(diary.name, user.name);
//		strcpy(diary.things, "try to LogIn");
		time(&timep);		
 //  		strcpy(diary.time,  ctime(&timep));	
 //  		write(FD, &diary, sizeof(diary));	
